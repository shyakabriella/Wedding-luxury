...
export default function ResortSectionThree() {
  ...
  useEffect(() => {
    const fetchSlides = async () => {
      try {
        const response = await fetch(`${API_URL}/wedding/section2/slides`);
        const result = await response.json();
        
        if (result.success && result.data.length > 0) {
          // Process slides and add formatted id
          const processedSlides = result.data.map((slide, index) => ({
            id: String(index + 1).padStart(2, "0"),
            eyebrow: slide.title,
            title: slide.subtitle,
            description: slide.description,
            image: slide.image_url && !slide.image_url.startsWith("http") 
              ? `${STORAGE_URL}/${slide.image_url.replace(/^\/storage\//, '')}` 
              : slide.image_url,
            display_order: slide.display_order,
          }));
          
          // Sort by display_order
          const sortedSlides = processedSlides.sort((a, b) => (a.display_order || 0) - (b.display_order || 0));
          setSlides(sortedSlides);
        } else {
          // Fallback data if no slides in API
          setSlides([
            {
              id: "01",
              eyebrow: "Why Choose Luxury Garden Palace?",
            ...
            },
            {
              ...
              eyebrow: "Why Choose Luxury Garden Palace?",
              title: "Easy to Enjoy",
              description: "From group retreats and business meetings to unforgettable celebrations, our resort offers flexible spaces, beautiful surroundings, and a warm atmosphere your guests will truly enjoy.",
              image: "/images/garden1.png",
            },
            {
              id: "03",
              eyebrow: "Why Choose Luxury Garden Palace?",
              title: "Easy to Remember",
              description: "Create meaningful experiences with elegant venues, comfortable accommodations, and outdoor moments that leave a lasting impression on every member of your group.",
              image: "/images/garden5.png",
            },
          ]);
        }
        ...
        setSlides([
          {
            ...
            eyebrow: "Why Choose Luxury Garden Palace?",
            title: "Easy to Plan",
            description: "Our seamless event planning experience includes versatile venues for 1,000+ guests...",
            image: "/images/exterior2.JPG",
          },
          {
            id: "02",
            eyebrow: "Why Choose Luxury Garden Palace?",
            title: "Easy to Enjoy",
            description: "From group retreats and business meetings to unforgettable celebrations...",
            image: "/images/garden1.png",
          },
          {
            id: "03",
            eyebrow: "Why Choose Luxury Garden Palace?",
            title: "Easy to Remember",
            description: "Create meaningful experiences with elegant venues...",
            image: "/images/garden5.png",
          },
        ]);
      } finally {
    ...
    return (
      ...
      >
        <div className="text-center text-gray-500">
          <p>No slides available. Please add content in the admin dashboard.</p>
        </div>
      </section>
    );
  }
          ...
          <div className="grid min-h-[560px] grid-cols-1 bg-[#f8f8f6] shadow-[0_15px_45px_rgba(0,0,0,0.08)] lg:min-h-[620px] lg:grid-cols-[1fr_1.02fr]">
            ...
            <div className="relative min-h-[320px] sm:min-h-[420px] lg:min-h-full">
              <img
                ...
                className="h-full w-full object-cover"
                onError={(e) => {
                  e.target.src = "/images/garden5.png";
                }}
              />
            </div>
          </div>
